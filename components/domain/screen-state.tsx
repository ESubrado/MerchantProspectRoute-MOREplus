...
export function ScreenState({ action, description, title, type }: ScreenStateProps) {
  return (
    <Card className={cn("min-h-52", stateStyles[type])}>
      <CardHeader>
        {type === "loading"
          ? <LoadingIndicator className="size-9 rounded-lg bg-[var(--primary-soft)]" />
          : <span aria-hidden="true" className={cn("grid size-9 place-items-center rounded-lg text-sm font-bold", type === "error" ? "bg-white text-[var(--danger)]" : "bg-[var(--primary-soft)] text-[var(--primary)]")}>{type === "error" ? "!" : "0"}</span>}
