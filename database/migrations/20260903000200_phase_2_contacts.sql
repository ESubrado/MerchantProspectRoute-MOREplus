-- Fresh/reset database baseline: Phase 2 Contacts.
-- Apply this phase-level migration only to a new or reset database, in filename order.
begin;

-- Consolidated from 20260903000300_phase_2_contacts_commands.sql.
...
-- Consolidated from 20260903000600_phase_2_lead_lifecycle_fields.sql.
-- Phase 2 adds the lead lifecycle, communication preference, and reply-classification fields used by Contacts.
-- Keep these fields nullable only where the behavioral contract permits it. Existing contacts retain a NULL creator
-- because a historical row cannot safely be attributed to the user applying this migration.
alter table public.leads
  add column created_by uuid references auth.users (id) on delete set null,
  add column email_dnc boolean not null default false,
  add column sms_dnc boolean not null default false,
  add column call_dnc boolean not null default false,
  add column reply_temperature integer check (reply_temperature between 0 and 4),
  add column stage text not null default 'new' check (btrim(stage) <> ''),
  add column status text not null default 'active' check (btrim(status) <> '');

comment on column public.leads.created_by is 'Authenticated user that created the contact when known.';
comment on column public.leads.email_dnc is 'The contact must not receive email outreach.';
comment on column public.leads.sms_dnc is 'The contact must not receive SMS outreach.';
comment on column public.leads.call_dnc is 'The contact must not receive call outreach.';
comment on column public.leads.reply_temperature is 'Reply classification: 0 neutral, 1 interested, 2 auto-reply, 3 DNC, 4 reschedule.';
comment on column public.leads.stage is 'Free-text CRM lifecycle stage.';
comment on column public.leads.status is 'Free-text CRM lifecycle status.';

-- PostgreSQL cannot replace a function when either its argument list or returned row type changes.
drop function public.crm_search_contacts(uuid, text, text, integer, integer);
drop function public.crm_create_contact(uuid, text, uuid, text);
drop function public.crm_update_contact(uuid, uuid, text, uuid, text);

-- The directory still returns only tenant-authorized records, now including the lifecycle and preference state needed by the drawer.
create function public.crm_search_contacts(
  p_workspace_id uuid,
  p_search text default '',
  p_filter text default 'all',
  p_limit integer default 25,
...
begin
  ...
  select
    ...
    contact_rows.primary_email,
    contact_rows.is_assigned,
    contact_rows.created_by,
    contact_rows.email_dnc,
    contact_rows.sms_dnc,
    contact_rows.call_dnc,
    contact_rows.reply_temperature,
    contact_rows.stage,
    contact_rows.status,
    contact_rows.created_at,
    contact_rows.updated_at,
...
-- Contact creation captures the actor, protects communication preferences, and records the complete editable field set.
create function public.crm_create_contact(
  p_workspace_id uuid,
  p_full_name text,
  p_company_id uuid default null,
  p_primary_email text default null,
  p_email_dnc boolean default false,
  p_sms_dnc boolean default false,
  p_call_dnc boolean default false,
...
begin
  ...
  values (
    ...
    normalized_name,
    auth.uid(),
    coalesce(p_email_dnc, false) or coalesce(p_reply_temperature = 3, false),
    coalesce(p_sms_dnc, false),
    coalesce(p_call_dnc, false),
    p_reply_temperature,
    normalized_stage,
    normalized_status
  )
  ...
  values (
    ...
    jsonb_build_object(
      'fields',
      jsonb_build_array(
        'name', 'company', 'primary_email', 'email_dnc', 'sms_dnc', 'call_dnc',
        'reply_temperature', 'stage', 'status'
      )
    )
  );
...
-- Updates keep the same tenant and manager checks, and preserve the original creator attribution.
create function public.crm_update_contact(
  p_workspace_id uuid,
  p_contact_id uuid,
  p_full_name text,
  p_company_id uuid default null,
...
begin
  ...
  set full_name = normalized_name,
      company_id = p_company_id,
      email_dnc = coalesce(p_email_dnc, false) or coalesce(p_reply_temperature = 3, false),
      sms_dnc = coalesce(p_sms_dnc, false),
      call_dnc = coalesce(p_call_dnc, false),
      reply_temperature = p_reply_temperature,
...
revoke all on function public.crm_search_contacts(uuid, text, text, integer, integer) from public;
revoke all on function public.crm_create_contact(uuid, text, uuid, text, boolean, boolean, boolean, integer, text, text) from public;
revoke all on function public.crm_update_contact(uuid, uuid, text, uuid, text, boolean, boolean, boolean, integer, text, text) from public;
grant execute on function public.crm_search_contacts(uuid, text, text, integer, integer) to authenticated;
grant execute on function public.crm_create_contact(uuid, text, uuid, text, boolean, boolean, boolean, integer, text, text) to authenticated;
grant execute on function public.crm_update_contact(uuid, uuid, text, uuid, text, boolean, boolean, boolean, integer, text, text) to authenticated;

comment on function public.crm_search_contacts(uuid, text, text, integer, integer) is 'Tenant-checked paginated Contacts directory read model, including lifecycle and preference state.';
comment on function public.crm_create_contact(uuid, text, uuid, text, boolean, boolean, boolean, integer, text, text) is 'Atomic owner-or-admin contact creation with communication preferences, lifecycle state, and an audit event.';
comment on function public.crm_update_contact(uuid, uuid, text, uuid, text, boolean, boolean, boolean, integer, text, text) is 'Atomic owner-or-admin contact update with communication preferences, lifecycle state, and an audit event.';


-- Consolidated from 20260903000700_phase_2_contact_name_parts.sql.
-- Phase 2 accepts first and last names at the Contacts boundary while preserving full_name as the directory display value.
-- PostgreSQL cannot replace a function when its argument list or returned row type changes.
drop function public.crm_search_contacts(uuid, text, text, integer, integer);
drop function public.crm_create_contact(uuid, text, uuid, text, boolean, boolean, boolean, integer, text, text);
drop function public.crm_update_contact(uuid, uuid, text, uuid, text, boolean, boolean, boolean, integer, text, text);
...
begin
  ...
  select
    contact_rows.id,
    contact_rows.full_name,
    contact_rows.first_name,
    contact_rows.last_name,
    contact_rows.company_id,
    contact_rows.company_name,
    contact_rows.primary_email,
    contact_rows.is_assigned,
...
begin
  ...
  values (
    ...
    jsonb_build_object(
      'fields',
      jsonb_build_array(
        'first_name', 'last_name', 'company', 'primary_email', 'email_dnc', 'sms_dnc',
        'call_dnc', 'reply_temperature', 'stage', 'status'
      )
    )
  );
...
revoke all on function public.crm_search_contacts(uuid, text, text, integer, integer) from public;
revoke all on function public.crm_create_contact(uuid, text, text, uuid, text, boolean, boolean, boolean, integer, text, text) from public;
revoke all on function public.crm_update_contact(uuid, uuid, text, text, uuid, text, boolean, boolean, boolean, integer, text, text) from public;
grant execute on function public.crm_search_contacts(uuid, text, text, integer, integer) to authenticated;
grant execute on function public.crm_create_contact(uuid, text, text, uuid, text, boolean, boolean, boolean, integer, text, text) to authenticated;
grant execute on function public.crm_update_contact(uuid, uuid, text, text, uuid, text, boolean, boolean, boolean, integer, text, text) to authenticated;

comment on function public.crm_search_contacts(uuid, text, text, integer, integer) is 'Tenant-checked paginated Contacts directory read model, including contact name parts, lifecycle, and preference state.';
comment on function public.crm_create_contact(uuid, text, text, uuid, text, boolean, boolean, boolean, integer, text, text) is 'Atomic owner-or-admin contact creation from first and last name inputs with communication preferences, lifecycle state, and an audit event.';
comment on function public.crm_update_contact(uuid, uuid, text, text, uuid, text, boolean, boolean, boolean, integer, text, text) is 'Atomic owner-or-admin contact update from first and last name inputs with communication preferences, lifecycle state, and an audit event.';
