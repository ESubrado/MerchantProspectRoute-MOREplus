-- Phase 5.1 establishes one real campaign for each workspace while retaining a deliberate future path to many.
...
comment on table public.campaigns is
  'The one and only campaign in a workspace for Phase 5.1. Relax campaigns_one_per_workspace_key only in a deliberate multi-campaign release.';
...
comment on table public.campaign_sequence_schedules is
  'Per-sequence scheduling configuration. Empty windows mean that the draft has no dispatchable schedule; Phase 5.1 creates no scheduler.';
...
comment on table public.campaign_sequence_steps is
  'Ordered campaign-sequence steps. Actual delivery execution is intentionally outside Phase 5.1.';
