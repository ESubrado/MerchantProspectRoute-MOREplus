...
test("Phase 5 creates exactly one campaign per workspace during bootstrap and under concurrent resolution", () => {
...
test("Phase 5 applies RLS and composite campaign ownership constraints to tenant outreach records", () => {
  for (const table of [
    "campaigns",
    "campaign_sequences",
...
test("Phase 5 keeps the shared campaign mailbox pool and blocks concurrent active enrollment", () => {
...
test("architecture, plan, and gap register state the invariant and deliberate multi-campaign path", () => {
  ...
  assert.match(gapRegister, /Phase 5 campaign boundary/i);
});
